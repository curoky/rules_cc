...
load("@com_curoky_rules_pkg//:recipes/configs.bzl", "configs")
...
def pkg_rules_dependencies():
    ...
    for key in configs:
        ...
        repo_tyep = config["type"]
        config.pop("type")
        if repo_tyep == "git_repository":
            git_repository(**config)
        else:
            config["build_file"] = "@com_curoky_rules_pkg//:recipes/{}/{}".format(key, config["build_file"])
            if repo_tyep == "new_local_repository":
                config.pop("branch")
                native.new_local_repository(**config)
            elif repo_tyep == "new_git_repository":
